...
func (c client) isUpdateAvailable(current *v6.Description, candidate *LatestDocument) (*Archive, error) {
	...
	log.Debugf("found database update candidate: %s", candidate.Archive.Description)
...
func isSupersededBy(m *v6.Description, other v6.Description) bool {
	...
	otherModel, otherOk := other.SchemaVersion.Model()
	currentModel, currentOk := m.SchemaVersion.Model()

	if !otherOk {
		log.Errorf("existing database has no schema version, doing nothing...")
		return false
	}

	if !currentOk {
		log.Errorf("update has no schema version, doing nothing...")
		return false
	}

	if otherModel != currentModel {
		log.Debugf("update is for a different DB schema, skipping...")
		return false
	}
